    ...
    <div>
        <el-card>
            <el-tabs v-model="queryData.tab" @tab-change="getOrderList">
                ...
                <el-tab-pane label="待發貨" name="noship"></el-tab-pane>
                <el-tab-pane label="待收貨" name="shiped"></el-tab-pane>
                <el-tab-pane label="已收貨" name="received"></el-tab-pane>
                <el-tab-pane label="已完成" name="finish"></el-tab-pane>
                <el-tab-pane label="已關閉" name="closed"></el-tab-pane>
                <el-tab-pane label="退款中" name="refunding"></el-tab-pane>
            </el-tabs>
            <el-row :gutter="30">
                <el-col :span="6">
                    <el-input placeholder="請輸入訂單號" clearable v-model="queryData.no" @clear="getOrderList">
            ...
            <el-row style="margin-top:15px">
                <el-col>
                    <el-button type="danger" size="small" @click="delOrderHandle">批量刪除</el-button>
                    <el-button type="primary" size="small" @click="openDialog">導出訂單</el-button>
                </el-col>
            </el-row>
            <el-table :data="tableData" style="width: 100%;margin-bottom:20px" @selection-change="selectOrders" border
                ...
                <el-table-column label="商品信息">
                    <template #default="scope">
                        <div class="goodsTitle">
                            <span style="color:blue;">訂單號：{{ scope.row.no }}</span>
                            <span>下單時間：{{ scope.row.update_time }}</span>
                ...
                <el-table-column prop="total_price" label="實付款" align="center" />
                <el-table-column prop="user.username" label="購買會員" align="center" />



                <el-table-column label="交易狀態">
                    <template #default="{ row }">
                        <div style="margin-bottom: 15px;">
                            付款狀態：
                            <el-tag v-if="row.payment_method == 'wechat'" type="success" size="small">微信支付</el-tag>
                            <el-tag v-else-if="row.payment_method == 'alipay'" size="small">支付寶支付</el-tag>
                            <el-tag v-else type="info" size="small">未支付</el-tag>
                        </div>
                        <div style="margin-bottom: 15px;">
                            發貨狀態：
                            <el-tag :type="row.ship_data ? 'success' : 'info'" size="small">{{ row.ship_data ? '已發貨' : '未發貨'
                            }}</el-tag>
                        </div>
                        <div>
                            收貨狀態：
                            <el-tag :type="row.ship_status == 'received' ? 'success' : 'info'" size="small">{{
                                row.ship_status == 'received' ? '已收貨' : '未收貨' }}</el-tag>
                        </div>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" align="center">
                    <template #default="{ row }">
                        <el-tag style="margin-right: 15px;cursor: pointer;" @click="openOrderDetails(row)">訂單詳情</el-tag>
                        <el-tag v-if="queryData.tab == 'noship'" type="warning" style="cursor: pointer;"
                            @click="orderSend(row.id)">訂單發貨</el-tag>
                        ...
                        <el-tag type="warning" v-if="queryData.tab == 'refunding'" style="cursor: pointer;margin-top:15px"
                            @click="refundHandle(row.id, 0)">拒絕退款</el-tag>
                    </template>
                </el-table-column>
            </el-table>
        </el-card>
        ...
        <!-- 拒絕退款 -->
        <el-dialog v-model="dialogVisibleNo" title="拒絕理由" width="40%">
        ...
        <!-- 導出訂單 -->
        <el-dialog v-model="dialogVisible" title="導出訂單" width="40%">
            <el-form :model="formData" label-width="120px">
                <el-form-item label="訂單類型">
                    <el-select v-model="formData.tab" placeholder="請選擇訂單狀態">
                        <el-option v-for="(item, i) in orderStatus" :key="i" :label="item.name" :value="item.value" />
                    </el-select>

                </el-form-item>
                <el-form-item label="時間範圍">
                    <el-date-picker v-model="formData.time" type="daterange" range-separator="To" start-placeholder="開始時間"
                        end-placeholder="結束時間" value-format="YYYY-MM-DD" />
        ...
        <!-- 訂單詳情 -->
        <el-dialog v-model="dialogVisibleOrder" title="訂單詳情" width="40%">
            <div class="orderMian">
                <div>
                    <div class="orderTop">訂單信息</div>
                    <div class="orderCon">
                        訂單號：{{ orderInfo.data.no }}<br>
                        付款方式：{{ orderInfo.data.payment_method }}<br>
                        下單時間：{{ orderInfo.data.update_time }}
                    </div>
                </div>
                <div>
                    ...
                    <div class="orderCon">
                        <div v-for="(item, i) in orderInfo.data.order_items" :key="i" style="border-bottom: 1px dashed #dbdbdb;padding-bottom: 13px;margin-bottom: 10px;">
                            商品名稱：{{ item.goods_item ? item.goods_item.title : '已刪除' }}<br>
                            商品價格：{{ item.price }}<br>
                            商品數量：{{ item.num }}<br>
                        ...
                        商品總價：{{ orderInfo.data.total_price }}
                    </div>

                </div>
                <div v-if="orderInfo.data.ship_data">
                    <div class="orderTop">發貨信息</div>
                    <div class="orderCon">
                        物流公司：{{ orderInfo.data.ship_data.express_company }}<br>
                        運單號： {{ orderInfo.data.ship_data.express_no }}<br>
                        <el-button type="primary" size="small" @click="openLgstc(orderInfo.data.id)">查看物流</el-button>

                    </div>
                </div>
                <div v-if="orderInfo.data.address">
                    <div class="orderTop">收貨信息</div>
                    <div class="orderCon">
                        收貨人：{{ orderInfo.data.address.name }}<br>
                        聯繫方式： {{ orderInfo.data.address.phone }}<br>
                        聯繫地址： {{ orderInfo.data.address.province + orderInfo.data.address.city +
        ...
        <el-dialog v-model="dialogVisibleSend" title="發貨" width="40%">
            <el-form :model="formOrderSend">
                <el-form-item label="快遞公司">
...
const openOrderDetails = (row) => {
    ...
    row.order_items = row.order_items.map(item => {
        if (item.skus_type == 1 && item.goods_skus) {
            let arr = []
            //多規格
            //重點是 循環遍歷對象 ，獲取每個對象的value值，並追加到數組
            for (const key in item.goods_skus.skus) {
                //主要獲取每個對象的value值 注意：key是 0 1                
